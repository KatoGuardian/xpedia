...
<svelte:head>
  <title>{rul.str(article.title || 'BootyPedia')}</title>
</svelte:head>
...
{#if !(article.id in rul.units) || article.id == "STR_GOLDEN_SAINT_1" || article.id == "STR_STAR_GOD_PUBLIC_ENEMY"}
